...
void DancingGuiLisener::buttonHit(Button *button)
{
    ...
    {
        ConnectManager::GetInstance()->Send("ready");
    }
...
{
    if (menu->getName() == "S_DanceScence"  )
    {
        //OgreBites::DecorWidget *tempSceneView = mTrayMgr->createDecorWidget(OgreBites::TL_NONE, "temp", "SdkTrays/Street");
        OgreBites::DecorWidget *tempDecorWidget = (OgreBites::DecorWidget *)mTrayMgr->getWidget("temp");
        tempDecorWidget->getOverlayElement()->setMaterialName("SdkTrays/" + menu->getSelectedItem()); //"SdkTrays/Street"
