...
void ConnectManager::Send(string tempstring, string tempstring2) //use for send message   ,it will appear in a main loop
{
    ...
    if (strcmp(tempstring.c_str(), "logoff") == 0)
    {
        ...
        }
        {
            ...
            //myDocument.SaveFile("myhouse.xml");//fux doubt
        }
    ...
    if (strcmp(tempstring.c_str(), "ready") == 0)
    {
        mTypeId = CS_READY;
        RakNet::BitStream mBitStream ;
        mBitStream.Write(mTypeId);
...
void ConnectManager::Receive(void)//use for receive message   ,it will appear in a main loop
{
    for (m_packetPtr = m_clientPtr->Receive(); m_packetPtr; m_clientPtr->DeallocatePacket(m_packetPtr), m_packetPtr = m_clientPtr->Receive())
    {
        ...
        case  SC_LOGOFF_HOUSE://用于局域网模式服务器向自己通知自己所在的房间内有某某加入
            ...
            break;
        case  SC_READY://用于局域网模式服务器向自己通知自己所在的房间内有某某加入
            this->handleReady();
            break;
        case  SC_UNREADY://用于局域网模式服务器向自己通知自己所在的房间内有某某加入
            this->handleUnReady();
            break;
...
void ConnectManager::handleReady()
{
    if (DancingGuiSys::GetInstance()->getIsMutiReady())
    {
        ;// DancingTimer::timerForReady->reset();;//DancingGuiSys::GetInstance()->setIsMutiReady();
    }
    else
    {
        DancingGuiSys::GetInstance()->setIsMutiOtherReady(true);
        DancingTimer::timerForReady->reset();
    }

}
void ConnectManager::handleUnReady()
{
...
void ConnectManager::handleLoginRequsetTwo(RakNet::BitStream *tempBitStream)//SC_LOGIN_HOUSE
{
    ...
    PersonElement->SetAttribute("shoe", tempRoleShoe.c_str() );

    myDocument.SaveFile("otherrole.xml");
	 DancingGuiSys::GetInstance()->setIsHaveOtherRole(true);
...
void ConnectManager::handleLoginRequsetOne(RakNet::BitStream *tempBitStream)//SC_LOGIN_HOUSE
{
    ...
    /*******************************************************************
    * 说    明：下面的设置函数本应该在界面的设置函数中setwidget中，但是如果直接转过去则不会顺利读取到myhouse文件而造成错误
    * 作    者： grius
    * 日    期：2013年3月27日
    *******************************************************************/
    DancingGuiSys::GetInstance()->setCurrentTypeChanged(true);
...
void ConnectManager::handleOtherRoleLeave()
{
    ...
    DancingGuiSys::GetInstance()->setIsHaveOtherRole(false);
}